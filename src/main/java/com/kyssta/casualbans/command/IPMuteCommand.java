...
public class IPMuteCommand extends BaseCommand implements CommandExecutor, TabCompleter {

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!checkPermission(sender, "casualbans.ipmute")) return true;

        if (args.length < 1) {
            sendUsage(sender, "/ipmute <player> [-s] [-S] [-p] [server:scope] [duration] [reason]");
            return true;
        }

        CommandFlags.ParsedResult parsed = parseFlags(args);
        CommandFlags flags = parsed.flags();
        flags.setIpPunish(true); // Force IP flag
        ...
        if (isExempt(targetUUID, "mute", sender)) {
            MessageUtil.sendError(sender, "That player is exempt from being IP-muted.");
            return true;
        }
        ...
        if (duration > 0) {
            long maxDuration = plugin.getStaffManager().getMaxDuration(sender, PunishmentType.IPMUTE);
            if (maxDuration > 0 && duration > maxDuration) {
                MessageUtil.sendError(sender, "Duration exceeds your maximum allowed IP-mute duration (" +
        ...
        if (!plugin.getStaffManager().hasCooldownBypass(sender)) {
            int cooldown = plugin.getStaffManager().getCooldownSeconds(sender, PunishmentType.IPMUTE);
            if (cooldown > 0) {
        ...
        plugin.getPunishmentManager().punish(sender, targetUUID, targetName,
            PunishmentType.IPMUTE, duration, reason, flags);
