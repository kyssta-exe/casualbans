...
public class IPBanCommand extends BaseCommand implements CommandExecutor, TabCompleter {

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!checkPermission(sender, "casualbans.ipban")) return true;

        if (args.length < 1) {
            sendUsage(sender, "/ipban <player> [-s] [-S] [-p] [server:scope] [duration] [reason]");
            return true;
        }
        ...
        if (isExempt(targetUUID, "ban", sender)) {
            MessageUtil.sendError(sender, "That player is exempt from being IP-banned.");
            return true;
        }
        ...
        if (duration > 0) {
            long maxDuration = plugin.getStaffManager().getMaxDuration(sender, PunishmentType.IPBAN);
            if (maxDuration > 0 && duration > maxDuration) {
                MessageUtil.sendError(sender, "Duration exceeds your maximum allowed IP-ban duration (" +
        ...
        if (!plugin.getStaffManager().hasCooldownBypass(sender)) {
            int cooldown = plugin.getStaffManager().getCooldownSeconds(sender, PunishmentType.IPBAN);
            if (cooldown > 0) {
        ...
        plugin.getPunishmentManager().punish(sender, targetUUID, targetName,
            PunishmentType.IPBAN, duration, reason, flags);
