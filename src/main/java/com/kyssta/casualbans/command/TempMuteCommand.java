...
public class TempMuteCommand extends BaseCommand implements CommandExecutor, TabCompleter {

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!checkPermission(sender, "casualbans.tempmute")) return true;

        if (args.length < 2) {
            sendUsage(sender, "/tempmute <player> [-s] [-S] [-p] [-I] [server:scope] <duration> [reason]");
            return true;
        }
        ...
        if (duration <= 0) {
            MessageUtil.sendError(sender, "Invalid duration. Use format like: 1d, 2h, 30m, 1w");
            return true;
        }

        // Check duration limits
        long maxDuration = plugin.getStaffManager().getMaxDuration(sender, PunishmentType.TEMPMUTE);
        if (maxDuration > 0 && duration > maxDuration) {
            MessageUtil.sendError(sender, "Duration exceeds your maximum allowed tempmute duration (" +
        ...
        if (!plugin.getStaffManager().hasCooldownBypass(sender)) {
            int cooldown = plugin.getStaffManager().getCooldownSeconds(sender, PunishmentType.TEMPMUTE);
            if (cooldown > 0) {
        ...
        plugin.getPunishmentManager().punish(sender, targetUUID, targetName,
            PunishmentType.TEMPMUTE, duration, reason, flags);
