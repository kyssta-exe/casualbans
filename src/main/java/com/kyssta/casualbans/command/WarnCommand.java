...
public class WarnCommand extends BaseCommand implements CommandExecutor, TabCompleter {

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!checkPermission(sender, "casualbans.warn")) return true;

        if (args.length < 1) {
            sendUsage(sender, "/warn <player> [-s] [-S] [-p] [server:scope] [reason]");
            return true;
        }
        ...
        // Check exempt
        if (isExempt(targetUUID, "warn", sender)) {
            MessageUtil.sendError(sender, "That player is exempt from being warned.");
            return true;
        }
        ...
        if (!plugin.getStaffManager().hasCooldownBypass(sender)) {
            int cooldown = plugin.getStaffManager().getCooldownSeconds(sender, PunishmentType.WARN);
            if (cooldown > 0) {
        ...
        plugin.getPunishmentManager().punish(sender, targetUUID, targetName,
            PunishmentType.WARN, 0, reason, flags);
